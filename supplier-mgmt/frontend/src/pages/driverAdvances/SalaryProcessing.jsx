...
export default function SalaryProcessing() {
  ...
  const canApprove = usePermission('driver_advances', 'approve');
  const now = new Date();
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const { data } = await fetchDriverSalarySummary({
        month: Number(month),
        year: Number(year),
      });
      setRows(data.data);
  ...
  const toggleAll = () => {
    if (selected.size === rows.length) {
      ...
      setSelected(new Set(rows.map((r) => r.driverId)));
    }
  };
  ...
  const handleProcess = async () => {
    ...
    const ok = await confirm({
      ...
      message: `Process salary for ${selected.size} driver(s) for ${label} ${year}? Pending advances will be marked as deducted. This cannot be undone.`,
    ...
    setProcessing(true);
    try {
      const { data } = await processDriverSalary({
        driverIds: [...selected],
        month: Number(month),
        year: Number(year),
      });
      ...
      <fieldset className="flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 bg-white p-4">
        ...
        {canApprove && (
          ...
          <Table
            ...
            error={loadError}
            onRetry={load}
            emptyMessage="No drivers with gross salary configured for this period."
            columns={[
              {
                key: 'select',
                label: (
            ...
            data={rows.map((r) => ({
              select: (
                <input
                  type="checkbox"
                  checked={selected.has(r.driverId)}
                  onChange={() => toggleOne(r.driverId)}
                  disabled={!canApprove}
                />
              ),
              name: r.name,
              ...
              _expanded: expanded === r.driverId ? r.advances : null,
              _rowKey: r.driverId,
            }))}
          />
          {rows.map(
